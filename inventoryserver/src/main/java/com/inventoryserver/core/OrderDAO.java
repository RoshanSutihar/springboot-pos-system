...
//	    PDF GENERATOR CODE
	    ...
	    public ByteArrayInputStream createonlinePDF(int orderId, String userid) {
	        ...
	        try {
	            ...
	            Paragraph ordercopy = new Paragraph("***************** COPY ********************** COPY ********************** COPY *****************");
	            ordercopy.setAlignment(Element.ALIGN_CENTER);
	            document.add(ordercopy);
	    ...
	    public ByteArrayInputStream createcopyPDF(int orderId, String userid) {
	        // Retrieve order details using getOrderById method
	        Order order = getOrderById(orderId);
	        if (order == null) {
	            System.out.println("Order not found.");
